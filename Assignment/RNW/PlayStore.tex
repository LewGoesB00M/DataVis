...
13.66\% of the dataset. To analyse this in further detail to see specifically which columns contain
...
\hlkwd{kable}\hlstd{(}\hlkwd{miss_var_summary}\hlstd{(dataDf),}
     \hlkwc{caption} \hlstd{=} \hlstr{"Missing variable summary of the dataset"}\hlstd{,} \hlstr{"latex"}\hlstd{)} \hlopt{%>%}
...
In this section, the questioned posed in Section \ref{sec:questions} will be answered
...
\hlkwd{ggplot}\hlstd{(filterReviews,} \hlkwd{aes}\hlstd{(}\hlkwc{x} \hlstd{= avgRating,} \hlkwc{y} \hlstd{= Category,} \hlkwc{fill} \hlstd{= Category))} \hlopt{+}
    ...
    \hlkwd{geom_text}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{label} \hlstd{=} \hlkwd{round}\hlstd{(avgRating,} \hlnum{3}\hlstd{)),}
              \hlkwc{position} \hlstd{=} \hlkwd{position_stack}\hlstd{(}\hlkwc{vjust} \hlstd{=} \hlnum{0.5}\hlstd{))} \hlopt{+}
    \hlkwd{labs}\hlstd{(}\hlkwc{title} \hlstd{=} \hlstr{"Average Rating by App Category"}\hlstd{,} \hlkwc{x} \hlstd{=} \hlstr{"App Category"}\hlstd{,}
         \hlkwc{y} \hlstd{=} \hlstr{"Average Rating"}\hlstd{)} \hlopt{+}
...
\hlstd{paidApps}\hlopt{$}\hlstd{priceFactor[}\hlopt{!}\hlstd{(paidApps}\hlopt{$}\hlstd{priceFactor} \hlopt{%in%} \hlstd{mostCommon)]} \hlkwb{<-} \hlstr{"Others"}
...
\hlkwd{filter}\hlstd{(numSize} \hlopt{!=} \hlstr{"0"} \hlopt{&} \hlstd{installThresholds} \hlopt{%in%} \hlkwd{c}\hlstd{(}\hlstr{"1 - 500"}\hlstd{,}
\hlstr{"500 - 1K"}\hlstd{,} \hlstr{"1K - 10K"}\hlstd{,}
\hlstr{"10K - 50K"}\hlstd{,} \hlstr{"50K - 100K"}\hlstd{,} \hlstr{"100K - 500K"}\hlstd{,}
\hlstr{"500K - 1M"}\hlstd{,} \hlstr{"1M - 10M"}\hlstd{))}
...
\begin{alltt}
\hlstd{doesNotVary} \hlkwb{<-} \hlstd{dataDf} \hlopt{%>%}
\hlcom{# Grab the other factor levels by filtering to those NOT in this list.}
\hlkwd{filter}\hlstd{(numSize} \hlopt{!=} \hlstr{"0"} \hlopt{& !}\hlstd{installThresholds} \hlopt{%in%} \hlkwd{c}\hlstd{(}\hlstr{"1 - 500"}\hlstd{,}
\hlstr{"500 - 1K"}\hlstd{,} \hlstr{"1K - 10K"}\hlstd{,}
\hlstr{"10K - 50K"}\hlstd{,} \hlstr{"50K - 100K"}\hlstd{,} \hlstr{"100K - 500K"}\hlstd{,}
\hlstr{"500K - 1M"}\hlstd{,} \hlstr{"1M - 10M"}\hlstd{))}
